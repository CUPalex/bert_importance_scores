...
class Pipeline:
    def __init__(self, task, batch_size, seed):
    ...
    def train_epoch(self):
        ...
        for batch in tqdm.tqdm(self.task.dataloader_train):
            ...
            wandb.log({"step/loss": batch_loss.item(),
                       "step/mse": torch.nn.functional.mse_loss(torch.argmax(predicted_logits, dim=-1), gold_outputs, reduction="mean").item()})
    ...
    def validate(self, dataloader, log_to_wandb=True):
        ...
        with torch.no_grad():
            for batch in tqdm.tqdm(dataloader):
                ...
                sum_mse += torch.nn.functional.mse_loss(torch.argmax(predicted_logits, dim=-1), gold_outputs, reduction="sum").item()
                sum_losses += batch_loss.item()
    ...
    def train(self, num_epochs=10):
        ...
        for epoch in range(num_epochs):
            self.train_epoch()
            self.validate(self.task.dataloader_val)
    ...
    def find_importance_scores(self):
        ...
        acc, loss, mae = self.validate(self.task.dataloader_test, log_to_wandb=False)
        wandb.log({"test_acc": acc, "test_loss": loss, "test_mae": mae})
        ...
        wandb.log({"accs_with_removal": wandb.Table(data=accs, columns=["attn_acc", "attn_loss", "attn_mae", "mlp_acc", "mlp_loss", "mlp_mae"])})
        wandb.finish()
