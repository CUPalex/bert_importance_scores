...
class Task:
    def __init__(self):
        ...
        self.dataset["train"] = self.dataset["train"].select([0, 1, 2, 3, 4, 5, 6])
        self.dataset["test"] = self.dataset["test"].select([0, 1, 2, 3, 4, 5, 6])
        self.dataset["validation"] = self.dataset["validation"].select([0, 1, 2, 3, 4, 5, 6])
    ...
    def create_datasets_and_dataloaders(self, task, tokenizer, batch_size):
        print(len(self.dataset["train"]), len(self.dataset["test"]), len(self.dataset["validation"]))
        ...
        self.num_classes = len(np.unique(self.dataset_train[task]))
        ...
        print(len(self.dataset_train), len(self.dataset_test), len(self.dataset_val))
